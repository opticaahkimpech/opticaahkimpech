  ...
  clientForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!clientId) {
        ...
        // Si el cliente se crea desde convenios, asociarlo al convenio
        if (convenioSource === "convenios" && convenio && empresaId) {
          // Obtener la empresa
          const empresaDoc = await getDoc(doc(db, "empresas", empresaId))
          if (empresaDoc.exists()) {
            const empresa = empresaDoc.data()
            ...
            const miembroData = {
              empresaId: empresaId,
              sucursal: empresa.sucursales ? empresa.sucursales[0] : "", // Tomar la primera sucursal por defecto
