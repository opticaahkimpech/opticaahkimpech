...
function showToast(message, type = "info", duration = 5000) {
  ...
  toast.className = `bg-white dark:bg-gray-800 shadow-lg rounded-lg p-4 mb-3 flex items-center justify-between border-l-4 transform translate-x-full transition-transform duration-300 ${
    type === "success"
      ? "border-green-500"
      : type === "danger"
        ? "border-red-500"
    ...
    confirmModal.innerHTML = `
            <div class="modal-content bg-white dark:bg-gray-800 w-11/12 md:w-1/3 max-w-md mx-auto mt-32 rounded-lg shadow-modal p-6">
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button id="confirmBtn" class="px-4 py-2 text-white rounded-md transition-colors ${
                      type === "danger"
...
async function validateClientName(name) {
  ...
  try {
    ...
    const clientesQuery = query(collection(db, "clientes"), where("nombre", "==", name))
...
function validateEmail(email) {
  ...
  if (!VALID_EMAIL_DOMAINS.includes(domain)) {
    setValidationState(
      ...
      errorDiv,
      false,
      `Dominio no válido. Use dominios como: ${VALID_EMAIL_DOMAINS.slice(0, 5).join(", ")}, etc.`,
    )
    return false
  }
...
function fillClientForm(clientData) {
  ...
  if (clientData.fechaNacimiento) {
    const fecha =
      clientData.fechaNacimiento instanceof Timestamp
...
async function buildAbonosPagosRows(venta) {
  ...
  // Abonos adicionales
  const abonosQuery = query(collection(db, "abonos"), where("ventaId", "==", venta.id), orderBy("fecha", "asc"))
  ...
  // Pagos
  const pagosQuery = query(collection(db, "pagos"), where("ventaId", "==", venta.id), orderBy("fecha", "asc"))
...
async function loadFullClientHistory(clientId) {
  try {
    ...
    // Obtener ventas del cliente
    const ventasQuery = query(collection(db, "ventas"), where("clienteId", "==", clientId), orderBy("fecha", "desc"))
...
async function loadVentaDetailsForFullHistory(ventaId, totalVenta, abonoInicial) {
  try {
    ...
    const abonosQuery = query(collection(db, "abonos"), where("ventaId", "==", ventaId), orderBy("fecha", "asc"))
    ...
    // Obtener pagos
    const pagosQuery = query(collection(db, "pagos"), where("ventaId", "==", ventaId), orderBy("fecha", "asc"))
...
window.toggleFullVentaDetails = (ventaId) => {
...
window.payAllPendingFromModal = async (clienteId, saldoTotal) => {
    ...
    try {
      ...
      for (const ventaDoc of ventasSnapshot.docs) {
        ...
        if (saldoInfo.saldoPendiente > 0) {
          ...
          await registrarPago(ventaDoc.id, clienteId, saldoInfo.saldoPendiente, "Pago total pendiente", "efectivo")
...
async function createConvenioMember(clienteId, empresaId, sucursal) {
  try {
    // Obtener información de la empresa para asignar el límite de crédito
    const empresaDoc = await getDoc(doc(db, "empresas", empresaId))
    let limiteCredito = 3500 // Valor por defecto

    if (empresaDoc.exists()) {
      const empresaData = empresaDoc.data()
      limiteCredito = empresaData.limiteCreditoPorMiembro || 3500
    }
    ...
    const miembroData = {
      ...
      sucursal: sucursal || "",
      clienteId: clienteId,
      limiteCredito: limiteCredito, // Asignar el límite de crédito de la empresa
      referencia: "",
    ...
    showToast(`Cliente asociado al convenio correctamente con límite de $${limiteCredito.toFixed(2)}`, "success")
...
function setupSearchEvents() {
  ...
  if (searchCliente) {
    ...
    searchCliente.addEventListener(
      "input",
      debounce(() => {
        performSearch()
      }, 500),
...
async function loadClientesWithOffset() {
  ...
  tableBody.innerHTML =
    '<tr><td colspan="6" class="py-4 text-center"><span class="spinner"></span> Cargando clientes...</td></tr>'
...
function applyMemoryFilters(clientes) {
  ...
  if (filtrosClientes.fechaRegistro) {
    ...
    filteredClientes = filteredClientes.filter((cliente) => {
      ...
      const fechaRegistro =
        cliente.fechaRegistro instanceof Timestamp ? cliente.fechaRegistro.toDate() : new Date(cliente.fechaRegistro)
  ...
  if (filtrosClientes.ultimaVisita) {
    const diasAtras = Number.parseInt(filtrosClientes.ultimaVisita)
    ...
    filteredClientes = filteredClientes.filter((cliente) => {
      ...
      const ultimaVisita =
        cliente.ultimaVisita instanceof Timestamp ? cliente.ultimaVisita.toDate() : new Date(cliente.ultimaVisita)
...
function showClientCard(cliente) {
  ...
  if (cliente.fechaNacimiento) {
    const fecha =
      cliente.fechaNacimiento instanceof Timestamp
...
async function loadClientHistory(clientId) {
  try {
    ...
    historyBody.innerHTML =
      '<tr><td colspan="4" class="py-4 text-center"><span class="spinner"></span> Cargando historial...</td></tr>'
    ...
    for (const ventaDoc of ventasSnapshot.docs) {
      ...
      const estadoClass = "text-yellow-500"
    ...
    if (historyBody) {
      historyBody.innerHTML =
        '<tr><td colspan="4" class="py-4 text-center text-red-500">Error al cargar historial</td></tr>'
    }
  }
}
...
async function calculateAndShowTotalPending(historyBody, clientId) {
  try {
    ...
    const ventasQuery = query(collection(db, "ventas"), where("clienteId", "==", clientId))
...
window.toggleVentaDetails = (ventaId) => {
...
function setupPaymentButtons(clientId) {
  ...
  document.querySelectorAll(".add-payment-btn").forEach((button) => {
    button.addEventListener("click", async (e) => {
      ...
      const saldoPendiente = Number.parseFloat(button.getAttribute("data-saldo"))
...
window.payAllPending = async (clienteId, saldoTotal) => {
  const confirmed = await showPaymentConfirmation(saldoTotal, "el pago total de todas las deudas pendientes")
  ...
  if (confirmed) {
    try {
      ...
      for (const ventaDoc of ventasSnapshot.docs) {
        ...
        if (saldoInfo.saldoPendiente > 0) {
          // Registrar pago completo para esta venta
          await registrarPago(ventaDoc.id, clienteId, saldoInfo.saldoPendiente, "Pago total pagado", "efectivo")
